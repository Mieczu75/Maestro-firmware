...
#include <cmath>  // Nadal potrzebne dla std::round, choć nie dla log10f

// Deklaracja funkcji mapFloat, aby była dostępna przed jej definicją na końcu pliku
// mapFloat będzie nadal potrzebne dla innych zastosowań, jeśli takie są w projekcie,
// ale nie do skalowania VU w tej wersji.
...
void VuWidget::init(WidgetConfig wconf, VUBandsConfig bands, uint16_t vumaxcolor, uint16_t vumincolor, uint16_t bgcolor) {
  ...
  _maxDimension = 216;  // Maksymalna szerokość/wysokość wskaźnika VU
...
void VuWidget::_draw() {
  ...
  // Statyczne zmienne, które przechowują bieżący poziom VU.
  ...
  // Odwrócenie wartości RAW. Zakładamy, że player.get_VUlevel zwraca
  // 255 dla ciszy i 0 dla maksymalnego sygnału.
  // L_current_linear_temp i R_current_linear_temp są teraz w zakresie 0-255,
  // gdzie 0 to cisza, a 255 to maksymalny sygnał.
  uint16_t L_current_linear_temp = 255 - L_raw;
  uint16_t R_current_linear_temp = 255 - R_raw;

  // --- KLUCZOWA ZMIANA: PRÓG CISZY NA POZIOMIE LINIOWYM ---
  // Jeśli liniowa wartość sygnału jest poniżej tego progu, traktujemy ją jako 0 (absolutna cisza).
  // Musisz DOSTOSOWAĆ TĘ WARTOŚĆ! Zwiększaj ją, jeśli w ciszy nadal pokazuje niezerowe wartości.
  const uint8_t VU_LINEAR_SILENCE_THRESHOLD = 39;  // Dostosuj tę wartość!

  uint16_t L_current_linear = (L_current_linear_temp < VU_LINEAR_SILENCE_THRESHOLD) ? 0 : L_current_linear_temp;
  uint16_t R_current_linear = (R_current_linear_temp < VU_LINEAR_SILENCE_THRESHOLD) ? 0 : R_current_linear_temp;
  // --- KONIEC PRÓGU CISZY ---

  // --- KLUCZOWA ZMIANA: LINIOWE SKALOWANIE BEZ LOGARYTMÓW ---
  // Bezpośrednie mapowanie wartości liniowej (0-255) na piksele wyświetlacza (0-_maxDimension).
  ...
  // Zabezpieczenie przed wartościami poza zakresem _maxDimension (0 do 216)
  ...
  // --- KONIEC LINIOWEGO SKALOWANIA ---

  // --- LOGIKA DLA WSKAŹNIKÓW SZCZYTU (MAX PEAK) ---
  // Jeśli kanały są powyżej progu ciszy, aktualizuj peak
  bool isAboveSilence = (L_current_linear > 0 || R_current_linear > 0);

  // Aktualizacja wskaźników szczytu tylko jeśli nie jesteśmy w ciszy
  if (isAboveSilence) {
    ...
    if (peakUpdated) {
      _peakFallDelayCounter = 0;  // Resetuj opóźnienie tylko jeśli któryś peak się zwiększył
    } else {
      _peakFallDelayCounter++;
    }

  } else {
    // Jeżeli cisza – zwiększamy licznik opóźnienia, ale nie resetujemy peaków
    _peakFallDelayCounter++;
  }

  // Opadanie wskaźników szczytu po upływie opóźnienia
  if (_peakFallDelayCounter >= _peakFallDelay) {
    if (_peakL > 0) _peakL -= _peakFallRate;
    if (_peakR > 0) _peakR -= _peakFallRate;
    _peakFallDelayCounter = 0;  // Resetuj licznik po opadnięciu
  }

  // Zabezpieczenie wskaźników szczytu przed wartościami ujemnymi i przekroczeniem max
  ...
  // --- KONIEC LOGIKI DLA WSKAŹNIKÓW SZCZYTU (MAX PEAK) ---

  // Aktualizacja bieżących mierników VU (measL_static, measR_static)
  bool played = player.isRunning();

  if (played) {
    // Jeżeli bieżący poziom jest większy, ustaw na bieżący. W przeciwnym razie, powoli opadaj.
    ...
    // Odtwarzacz zatrzymany, wskaźniki powinny szybko opadać do zera.
    if (measL_static > 0) measL_static -= _bands.fadespeed;
    if (measR_static > 0) measR_static -= _bands.fadespeed;
    _peakL = 0;  // Zresetuj wskaźniki szczytu, gdy odtwarzacz jest zatrzymany
    _peakR = 0;
    _peakFallDelayCounter = 0;  // Zresetuj licznik opóźnienia
  }

  // Zabezpieczenie mierników bieżących przed wartościami ujemnymi i przekroczeniem max
  ...
  // Rysowanie obramowania wskaźnika
  ...
  // --- KLUCZOWA ZMIANA: Etykiety dla wskaźnika liniowego ---
  // Możesz dostosować te etykiety, aby odzwierciedlały zakres liniowy,
  // np. procenty, wartości surowe (0-255), lub po prostu główne punkty.
  // Usunąłem również linie skali, aby nie było ich za dużo.

  // Pozycja dla 0% (początek wskaźnika)
  ...
  // Pozycja dla 100% (koniec wskaźnika)
  dsp.setCursor(_config.left + 12 + _maxDimension - 10, _config.top - 6);
  dsp.print("100");

  // Linie skali (pozostałe jako znaczniki, ale bez etykiet dB)
  // Dostosowałem offsety tak, aby były proste i bez mapFloat do dB
  dsp.drawLine(_config.left + 9, _config.top - 2, _config.left + 9, _config.top - 5, GRAY_9);  // 0%
  dsp.drawLine(_config.left + 9 + (int)(_maxDimension * 0.25), _config.top - 2,
               _config.left + 9 + (int)(_maxDimension * 0.25), _config.top - 5, GRAY_9);  // 25%
  dsp.drawLine(_config.left + 9 + (int)(_maxDimension * 0.50), _config.top - 2,
               _config.left + 9 + (int)(_maxDimension * 0.50), _config.top - 5, GRAY_9);  // 50%
  dsp.drawLine(_config.left + 9 + (int)(_maxDimension * 0.75), _config.top - 2,
               _config.left + 9 + (int)(_maxDimension * 0.75), _config.top - 5, GRAY_9);  // 75%
  dsp.drawLine(_config.left + 12 + _maxDimension, _config.top - 2,
               _config.left + 12 + _maxDimension, _config.top - 5, GRAY_9);  // 100%

  dsp.setFont();

  // Rysowanie segmentów tła VU (paski tła)
  // To może pozostać liniowe, chyba że chcesz, aby tło też miało szachownicę.
  // Zostawiłem jak jest, bo to tylko tło, a szachownica dotyczy "wypełnienia".
  ...
  // --- KLUCZOWA ZMIANA: RYSOWANIE WSKAŹNIKA VU (szachownica) ---
  // Należy usunąć istniejące dsp.fillRect i zastąpić je rysowaniem piksel po pikselu
  if (_config.align) {  // Jeśli wskaźnik jest poziomy
#ifndef BOOMBOX_STYLE
    // Wyczyść obszar wskaźnika przed rysowaniem szachownicy
    ...
    // Rysowanie szachownicy dla kanału L
    for (int x = 0; x < measL_static; x++) {
      for (int y = 0; y < _bands.height; y++) {
        // Szachownica: (x + y) % 2 == 0 dla jednego koloru, (x + y) % 2 != 0 dla drugiego
        if ((x + y) % 2 == 0) {
          dsp.drawPixel(_config.left + 12 + x, _config.top + y, GRAY_3);  // Kolor jasny szary
        } else {
          dsp.drawPixel(_config.left + 12 + x, _config.top + y, _bgcolor);  // Kolor czarny lub inny kontrastowy
        }
      }
    }
    // Rysowanie szachownicy dla kanału R
    for (int x = 0; x < measR_static; x++) {
      for (int y = 0; y < _bands.height; y++) {
        if ((x + y) % 2 == 0) {
          dsp.drawPixel(_config.left + 12 + x, _config.top + _bands.space + y, GRAY_3);
        } else {
...
#endif
  } else {  // Jeśli wskaźnik jest pionowy (domyślna konfiguracja)
    // Wyczyść obszar wskaźnika przed rysowaniem szachownicy
    dsp.fillRect(_config.left, _config.top, _bands.width, _maxDimension + _bands.space + _bands.height, _bgcolor);

    // Rysowanie szachownicy dla kanału L (od dołu do measL_static)
    for (int y_offset = 0; y_offset < measL_static; y_offset++) {
      int y = _config.top + _maxDimension - 1 - y_offset;  // Rysowanie od dołu do góry
      for (int x = 0; x < _bands.width; x++) {
        if ((x + y_offset) % 2 == 0) {  // Zmieniłem na y_offset, żeby wzór szedł w górę
          dsp.drawPixel(_config.left + x, y, GRAY_3);
        } else {
          dsp.drawPixel(_config.left + x, y, _bgcolor);
        }
      }
    }
    // Rysowanie szachownicy dla kanału R (od dołu do measR_static)
    for (int y_offset = 0; y_offset < measR_static; y_offset++) {
      int y = _config.top + _bands.space + _maxDimension - 1 - y_offset;  // Rysowanie od dołu do góry
      for (int x = 0; x < _bands.width; x++) {
        if ((x + y_offset) % 2 == 0) {
          dsp.drawPixel(_config.left + x, y, GRAY_3);
        } else {
  ...
  // --- RYSOWANIE WSKAŹNIKÓW SZCZYTU (MAX PEAK LDA) ---
  // Ta sekcja korzysta już z obliczonych wartości _peakL i _peakR,
  // które są liniowo wyskalowane.
  if (_config.align) {
    // Lewy kanał - linia szczytu
    dsp.fillRect(_config.left + 12 + _peakL, _config.top, 1, _bands.height, TFT_FG);
    // Prawy kanał - linia szczytu
    dsp.fillRect(_config.left + 12 + _peakR, _config.top + _bands.space, 1, _bands.height, TFT_FG);
  } else {
    // Lewy kanał - linia szczytu (od dołu)
    dsp.fillRect(_config.left, _config.top + _maxDimension - _peakL - 1, _bands.width, 1, TFT_FG);
    // Prawy kanał - linia szczytu (od dołu)
...
// Funkcja pomocnicza do mapowania wartości float
// Działa podobnie jak Arduino map(), ale dla liczb zmiennoprzecinkowych.
float mapFloat(float x, float in_min, float in_max, float out_min, float out_max) {
